...
public class FiltersAndMapExercise {
    ...
    public static  List<Integer> findEvenNumbers() {
        ...
        evenNumbers = input.filter(i -> i%2==0).collect(toList());
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter(s->s.equals("Jack")).collect(toList());
    ...
    public static List<Integer> filterNull(){
        ...
        evenNumbers = input.filter(i->i!=null).collect(toList());
        return evenNumbers;
    }
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age){
     ...
     persons = personList.stream().filter(p->
       p.getName().equals(name) && p.getAge().equals(age)).collect(toList());
        return persons;
    }
    ...
    public static List<Integer> flatteningLists() {
        ...
        together = input.flatMap(list-> list.stream()).collect(toList());
    ...
    public static int product(){
        ...
        //TODO #8
        return input.reduce(1,(product,x)->product*x);
    ...
    public static boolean areAllPersonsBelowAge(List<Person> personList, Integer age){
        ...
        overAge = personList.stream().allMatch(person -> person.getAge() < 70);
        return overAge;
    }
    ...
    public static int findTheMinimumNumber(){
        ...
        // HINT: use reduce
        int min ;
        min = input.reduce(Integer.MAX_VALUE,(min1,x)->Integer.min(min1,x));
        return min;
    }
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        int min ;
//       min = personList.stream().map(person -> person.getAge()).reduce(Integer.MAX_VALUE,Integer::min);
         min = personList.stream().collect(minBy(Comparator.comparingInt(Person::getAge))).get().getAge();
        return min;
    }
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList){
        ...
        // HINT use startsWith() for finding the names that starts with J, and distinct() to remove duplicates
            personList.stream().filter(person -> person.getName().startsWith("J")).map(person -> person.getNationality()).distinct();
        return  persons;
    }
    ...
    public static int sumStringLength(){
        ...
        int sum =0 ;
        //TODO #16
        sum = strings.stream().map(string -> string.length()).reduce(0,(acc,s)->acc+s);
        return sum;
    }
